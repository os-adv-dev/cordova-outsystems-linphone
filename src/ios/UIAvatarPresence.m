...
INIT_WITH_COMMON_CF {
	[NSNotificationCenter.defaultCenter addObserver:self
										   selector:@selector(onPresenceChanged:)
											   name:@"LinphoneNotifyPresenceReceivedForUriOrTel"
											 object:nil];
...
- (void)updatePresenceImage {
	...
	//LOGE(@"Friend %s status is now %s/%s since %@", _friend ? linphone_friend_get_name(_friend) : "NULL", basic == LinphonePresenceBasicStatusOpen ? "open" : "closed",activity ? linphone_presence_activity_to_string(activity) : "Unknown",[NSDate dateWithTimeIntervalSince1970:linphone_presence_model_get_timestamp(model)]);
