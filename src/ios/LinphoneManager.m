...
static void linphone_iphone_message_received_unable_decrypt(LinphoneCore *lc, LinphoneChatRoom *room,
	...
	NSString *strAddr = [NSString stringWithUTF8String:linphone_address_get_display_name(address)];
...
- (void)finishCoreConfiguration {
	...
	linphone_core_set_user_agent(theLinphoneCore, device.UTF8String, linphone_core_get_version());
...
- (void)startCallPausedLongRunningTask {
	...
	//LOGI(@"Long running task started, remaining [%@] because at least one call is paused",[LinphoneUtils intervalToString:[[UIApplication sharedApplication] backgroundTimeRemaining]]);
}
